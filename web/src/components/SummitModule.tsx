...
function VideoPanoramaLedge({
  ...
  const currentViewport = viewport.getCurrentViewport(camera, new Vector3(position[0], position[1], position[2]));
  ...
  useFrame((state, delta) => {
    if (meshRef.current && tex.image) {
      ...
      if (animProgress < 0.83) {
        opacity = Math.min(1, Math.max(0, (animProgress - 0.5) / 0.16));
      } else {
...
function ForegroundLedge({ textureUrl, position, startZ, endZ, startY, endY, scrollProgress }: ForegroundLedgeProps) {
  ...
  const currentViewport = viewport.getCurrentViewport(camera, new Vector3(position[0], position[1], endZ));
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      ...
      meshRef.current.position.z = MathUtils.lerp(startZ, endZ, easeOut);
